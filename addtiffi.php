...
$path="upload/";
				...
				$name=$pid;
			if((($ext=="jpg")||($ext=="jpeg"))&&($fsize<1200000))
			{										 
            ...
            $q=mysqli_query($con,"insert into add_tiffin(tname,tcategory,tqty,otime,tprice,timg)values('$tname', '$tcategory', '$tqty', '$otime', '$tprice','$k2')");

          
              if($q)
						  {
							  echo "<script language='javascript'>alert ('product added successfully ');</script>";
                echo "<script>window.location.href='apage.php';</script>";
						  }
						  else
						  {
					...
					{
						echo "<script language='javascript'>alert ('problem in image adding ');</script>";
					}
      }
      else
			{
